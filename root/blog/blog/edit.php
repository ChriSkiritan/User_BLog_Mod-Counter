...
{   
	$sql_data = array(
		...
		'blog_approved' 			=> (blog_data::$blog[$blog_id]['blog_approved'] == 1 || blog_data::$blog[$blog_id]['blog_approved'] == 0) && $auth->acl_get('u_blognoapprove'),
		'enable_bbcode' 			=> $post_options->enable_bbcode,
		'enable_smilies'			=> $post_options->enable_smilies,
	...
	if (blog_data::$blog[$blog_id]['blog_deleted'] == 0) 
	{
		...
		// Please don't change the order of the following two lines.
		put_blogs_in_cats($blog_id, $category_ids, true, 'approve');
		put_blogs_in_cats($blog_id, $category_ary);
		}

		if (blog_data::$blog[$blog_id]['blog_approved'] == 1 && $auth->acl_get('u_blognoapprove'))
		{
		...
		// Please don't change the order of the following two lines.
		put_blogs_in_cats($blog_id, $category_ids, true, 'approve');
		put_blogs_in_cats($blog_id, $category_ary, (($auth->acl_get('u_blognoapprove')) ? false : false));
		}
		
	handle_blog_cache('edit_blog', $user_id);

	$message = ((!$sql_data['blog_approved']) ? $user->lang['BLOG_NEED_APPROVE'] . '<br /><br />' : $user->lang['BLOG_EDIT_SUCCESS']) . '<br />';
