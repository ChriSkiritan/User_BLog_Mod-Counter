...
else // user submitted and there are no errors
{
	$sql_data = array(
		...
		'reply_approved' 		=> (blog_data::$reply[$reply_id]['reply_approved'] == 1 || blog_data::$reply[$reply_id]['reply_approved'] == 0) && $auth->acl_get('u_blogreplynoapprove'),
	...
	$message .= '<a href="' . $blog_urls['view_reply'] . '">' . $user->lang['VIEW_REPLY'] . '</a><br />';
		...
		if (blog_data::$reply[$reply_id]['reply_approved'] == 0 && $auth->acl_get('u_blogreplynoapprove'))
		{
			if (blog_data::$reply[$reply_id]['reply_deleted'] == 0) 
			{
				...
				$db->sql_query($sql);
				if ((blog_data::$blog[$blog_id]['blog_approved'] == 1) && (blog_data::$blog[$blog_id]['blog_deleted'] == 0))
				{		
					set_config('num_blog_replies', ++$config['num_blog_replies'], true);
				}
			}
		}
		if (blog_data::$reply[$reply_id]['reply_approved'] == 1 && !$auth->acl_get('u_blogreplynoapprove'))
		{
			if (blog_data::$reply[$reply_id]['reply_deleted'] == 0) 
			{
